...
#>
    [CmdletBinding()]
    param(
        [Parameter( Position = 0, Mandatory = $true, HelpMessage = 'Please provide base path to start search')]
        [ValidateScript( {Test-Path $_})]
        $BasePath,
        [Parameter( Position = 1)]
        [int]$Age = 0,
        [Parameter( Position = 2, HelpMessage='Please provide full path for output csv file')]
